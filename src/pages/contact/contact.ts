...
export class ContactPage {
  ...
  async send1() {
    ...
    await this.bluetoothSerial.write('MS0008C10D10E10C10C10D10E10C10');
    ...
    // MI (1), FÁ (1), SOL (2) 2x
    await this.bluetoothSerial.write('MS0006E10F10G20E10F10G20');
    await this.bluetoothSerial.read().then((data) => {
      if (data == 'ERRO') {
        // lancar uma excessao
        swal(
    ...
    //  SOL (1/2), LÁ (1/2), SI (1/2), FÁ (1/2), MI (1), DÓ (1) 2x
    await this.bluetoothSerial.write('MS0012G05A05B05F05E10C10G05A05B05F05E10C10');
    await this.bluetoothSerial.read().then((data) => {
      if (data == 'ERRO') {
        // lancar uma excessao
        swal(
    ...
    //  DÓ (1), SI (1), C2 (1) 2x
    await this.bluetoothSerial.write('MS0006C10B10C210C10B10C210');
    await this.bluetoothSerial.read().then((data) => {
      if (data == 'ERRO') {
        // lancar uma excessao
        swal(
  ...
  click(button) {
    return new Promise(resolve => {
      ...
      setTimeout(() => {
        document.getElementById(button).className = document.getElementById(button).className.replace("activated", "");
        resolve();
      }, 100, 1);
    });
  }   
